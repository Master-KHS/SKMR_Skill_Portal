...
사용자가 자연어로 질문하면 내부 Skill/Profile/Raw Data를 근거로 후보를 추천하고, 같은 질문 기준의 요약 대시보드를 즉시 함께 보여준다.
...
- 외부 논문, 연구실, 공개 프로필 검색은 사용자 승인 이후 실행한다.
...
즉 `씬필름 주니어 인력 추천` 같은 질문에서 박막/Photo 계열 키워드와 주니어 조건이 같이 해석되면 상위 후보로 노출될 수 있어야 한다.
...
- 연구실 이력도 논문과 동일하게 승인 후 외부 검색 범위에 포함한다.
...
- 학력/논문/연구실 정보는 별도 탭으로 분리

## 9. Gemini / 외부 보강 원칙

- Gemini는 보조 요약 역할만 수행한다.
- 실제 외부 논문/Google Scholar/연구실 페이지를 확인하지 않았으면 확인한 것처럼 말하지 않는다.
- 내부 Raw Data와 Skill Profile이 기본 근거다.
- 외부 보강 실패, quota 초과, auth 실패 시에도 내부 추천은 정상 동작해야 한다.
- 외부 논문/연구실 이력 실연동은 사용자 승인 후 실행한다.
...
3. 연구실/랩/공개 연구 프로필 이력까지 외부 근거 범위 확장
4. Skill 설명/사전 연결 강화
5. 후보 비교표 / 결과 export UX 보강
6. 조직/직무별 추천 근거 템플릿 정교화
