...
export function StageAssess({ title, desc, stage, members, confirmLabel, scope }: Props) {
  ...
  async function save() {
    ...
    const updates = rows.map((row) => ({
      ...
      confirmed_level: edits[row.skill_id] ?? row.stage_level ?? row.self_level ?? row.current_level,
    }));
    ...
    if (!res.ok) {
      setMsg(data.error ?? "저장 중 오류가 발생했습니다.");
      return;
    }

    setMsg(
      `${data.saved}건 ${confirmLabel} 완료 (${data.date})` +
        (stage === "committee" ? " · 최종 결과가 Skill Profile에 반영되었습니다." : "")
    );
    load();
  }
  ...
  for (const row of rows) {
    (groups.get(row.sub_family_name) ?? groups.set(row.sub_family_name, []).get(row.sub_family_name)!).push(row);
  }
  ...
  return (
    <div className="max-w-5xl">
      ...
      {[...groups.entries()].map(([subFamily, items]) => (
        <Card key={subFamily} title={subFamily} className="mb-4">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-border-soft text-left text-text-muted">
                ...
                <th className="w-20 py-2 pr-3">자가</th>
                <th className="w-24 py-2 pr-3">현재</th>
                <th className="w-56 py-2">{confirmLabel}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((row) => {
                const value = edits[row.skill_id] ?? row.stage_level ?? row.self_level ?? row.current_level;
                const changed = row.skill_id in edits;

                return (
                  <tr key={row.skill_id} className="border-b border-border-soft">
                    <td className="py-2 pr-3">{row.skill_name}</td>
                    <td className="py-2 pr-3 text-text-muted">{row.target_level ? `L${row.target_level}` : "-"}</td>
                    <td className="py-2 pr-3 text-text-muted">{row.self_level ? `L${row.self_level}` : "-"}</td>
                    ...
                    <td className="py-2">
                      <div className="flex items-center gap-2">
                        ...
                        {row.stage_level ? <Badge tone="success" label="확정됨" /> : null}
                      </div>
                      <div className="mt-1">
