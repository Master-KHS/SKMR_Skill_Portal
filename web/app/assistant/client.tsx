...
import { runAssistantClient, type AssistantMeta } from "@/lib/assistant-client";
...
export function AssistantClient({
  ...
  async function ask(q: string) {
    if (!q.trim()) return;
    if (!apiKey) {
      setError("Gemini 키가 설정되지 않았습니다. 실행 배치(시작-START.bat)의 GEMINI_API_KEY 를 확인하세요.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    setLastQuestion(q);
    try {
      // 브라우저에서 직접 Gemini 호출 (서버 아웃바운드 불필요)
      const data = await runAssistantClient(q, { apiKey, model, meta });
      setResp(data);
    } catch (e) {
  ...
  const verifyBadge = () => {
    if (!resp) return null;
    if (resp.verification === "pass") return <Badge tone="success" label="검증: Pass (결과 있음)" />;
    if (resp.verification === "fail") return <Badge tone="danger" label="검증: Fail (해당 인원 없음)" />;
    return <Badge tone="warning" label="검증: 확인 필요 (조건 불명확)" />;
  };

  return (
    <div className="max-w-4xl">
      <PageHeader title="AI 인재 검색" desc="자연어로 질문하면 스킬·인재 DB를 근거로 답합니다 (Gemini 연동)" />
